...
class RelatorioMembershipArrecadacaoController extends Controller{
    ...
    public function index(Request $request){
        ...
        $params['group_by'] = 'fk_assinatura';

        $this->arrayViewData['repasses'] = Assinatura::relatorio_arrecadacao_assinaturas($params)->paginate(20);

        if ($request->input('export') == 1){
            $data_export = Assinatura::relatorio_arrecadacao_assinaturas($params)->get();

            $this->export(['repasses' => $data_export], 'relatorio.membership.table_arrecadado');
        }
        
        $this->getFiltros();


        $this->arrayViewData['table'] = view('relatorio.membership.table_arrecadado', $this->arrayViewData);
        $this->arrayViewData['data_registro'] = $params['data_registro'];

        return view('relatorio.membership.arrecadado', $this->arrayViewData);
    }
