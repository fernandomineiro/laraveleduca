                        ...
                        @foreach($perfis as $perfil)
                            <tr>
                                ...
                                @foreach($parceiro as $p)
                                    @if($p->id == $perfil->fk_parceiro_id)
                                        <td style="text-align: center">{{ $p->descricao }}</td>
                                    @endif
                                @endforeach

                                <td>{{ $lista_status[$perfil->status] }}</td>
                                <td style="text-align: center">
                                    <a href="{{ $perfil->id }}/editar"
                                       class="btn btn-default btn-sm" title="Editar"><i
                                            class="fa fa-fw fa-edit"></i></a>
                                    {{ Form::open(['method' => 'DELETE', 'route' => ['admin.'.$modulo['moduloDetalhes']->rota.'.deletar', $perfil->id], 'style' => 'display:inline;']) }}
