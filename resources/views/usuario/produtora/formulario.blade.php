...
@section('content')
    <div class="box padding20">
        ...
        @if(Request::is('*/editar'))
            {{ Form::model( $objProdutora, ['method' => 'PATCH', 'route' => ['admin.'.$modulo['moduloDetalhes']->rota.'.atualizar', $objProdutora->id], 'files' => true, 'autocomplete' => 'off'] ) }}

            <input type="hidden" name="fk_usuario_id" value="<?php echo $objProdutora->fk_usuario_id; ?>" />
            <input type="hidden" name="id" value="<?php echo $objProdutora->id; ?>" />
            <input type="hidden" name="fk_endereco_id" value="<?php echo $objProdutora->fk_endereco_id; ?>" />
            <input type="hidden" name="fk_conta_bancaria_id" value="<?php echo $objProdutora->fk_conta_bancaria_id; ?>" />
        @else
        ...
        <div class="row">
            <div class="form-group col-md-4">
                {{ Form::label('Titular *') }}
                {{ Form::input('text', 'responsavel', isset($objProdutora->responsavel) ? $objProdutora->responsavel : null, ['class' => 'form-control', '', 'placeholder' => 'Titular', 'maxlength' => 30]) }}
            </div>
            <div class="form-group col-md-4">
                {{ Form::label('Data Nascimento *') }}
                {{ Form::input('text', 'data_nascimento', isset($objProdutora->data_nascimento) ? implode('/', array_reverse(explode('-', $objProdutora->data_nascimento))) : null, ['class' => 'form-control datepicker', '', 'placeholder' => 'Data Nascimento', 'maxlength' => 10]) }}
            </div>
            <div class="form-group col-md-4">
                {{ Form::label('CPF *') }} <span>(Digitar sem pontuação)</span>
                {{ Form::input('text', 'cpf', isset($objProdutora->cpf) ? $objProdutora->cpf : null, ['class' => 'form-control cpf', 'onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'CPF/CNPJ', 'maxlength' => 20]) }}
            </div>
        </div>
        <hr class="hr"/>
        ...
        <div class="row">
            <div class="form-group col-md-3">
                ...
                {{ Form::input('text', 'razao_social', isset($objProdutora->razao_social) ? $objProdutora->razao_social : null, ['class' => 'form-control', '', 'placeholder' => 'Razão Social', 'maxlength' => 90]) }}
            </div>
            <div class="form-group col-md-3">
                {{ Form::label('Nome Fantasia') }}
                <small>(Max 90 caracteres)</small>
                {{ Form::input('text', 'fantasia', isset($objProdutora->fantasia) ? $objProdutora->fantasia : null, ['class' => 'form-control', '', 'placeholder' => 'Nome Fantasia', 'maxlength' => 90]) }}
            </div>
            <div class="form-group  col-md-3">
                {{ Form::label('CNPJ') }}
                {{ Form::input('text', 'cnpj', isset($objProdutora->cnpj) ? $objProdutora->cnpj : null, ['class' => 'form-control cpf', 'onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'CNPJ', 'maxlength' => 20]) }}
            </div>
            {{--@if(!Request::is('*/editar'))
                <div class="form-group  col-md-3">
                    {{ Form::label('CNPJ') }}
                    {{ Form::input('text', 'cnpj', isset($objProdutora->fantasia) ? $objProdutora->fantasia : null, ['class' => 'form-control cpf', 'onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'CNPJ', 'maxlength' => 20]) }}
                </div>
            @endif
            @if(Request::is('*/editar'))
                <div class="form-group  col-md-3">
                    {{ Form::label('CNPJ *') }}
                    {{ Form::input('text', 'cnpj', $objProdutora->cnpj, ['class' => 'form-control cnpj', 'readonly' => 'readonly', 'onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'CNPJ', 'maxlength' => 20]) }}
                </div>
            @endif--}}
            <div class="form-group col-md-3">
                {{ Form::label('Representante Legal') }}
                <small>(Max 60 caracteres)</small>
                {{ Form::input('text', 'representante_legal', isset($objProdutora->representante_legal) ? $objProdutora->representante_legal : null, ['class' => 'form-control', '', 'placeholder' => 'Representante Legal', 'maxlength' => 60]) }}
            </div>
        </div>
        <hr class="hr"/>
        <div class="row">
            <div class="form-group col-md-2">
                {{ Form::label('% Share *') }}
                {{ Form::input('number', 'share', isset($objProdutora->share) ? $objProdutora->share : null, ['class' => 'form-control',  'min'=>'0,00', 'max'=>'100', 'step'=>'0.01', '', 'placeholder' => '0,00%']) }}
            </div>
            <div class="form-group  col-md-2">
                {{ Form::label('Telefone Fixo ') }}
                {{ Form::input('text', 'telefone_1', isset($objProdutora->telefone_1) ? $objProdutora->telefone_1 : null, ['class' => 'form-control telefone','onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'Telefone']) }}
            </div>
            <div class="form-group  col-md-2">
                {{ Form::label('Telefone Celular *') }}
                {{ Form::input('text', 'telefone_2', isset($objProdutora->telefone_2) ? $objProdutora->telefone_2 : null, ['class' => 'form-control celular','onkeypress'=>'return onlyNumbers(event)', '', 'placeholder' => 'Telefone']) }}
            </div>
        </div>
        <hr class="hr"/>
        <div class="row">
            @if(Request::is('*/editar'))
                {{ Form::input('hidden', 'fk_endereco_id', isset($objProdutora->fk_endereco_id) && isset($objProdutora->fk_endereco_id) ? $objProdutora->fk_endereco_id : null) }}
            @endif
            <div class="form-group col-md-2">
                {{ Form::label('CEP *') }}
        ...
        <div class="row">
            @if(Request::is('*/editar'))
                {{ Form::input('hidden', 'fk_conta_bancaria_id', isset($objProdutora->fk_conta_bancaria_id) && isset($objProdutora->fk_conta_bancaria_id) ? $objProdutora->fk_conta_bancaria_id : null) }}
            @endif
        ...
        <div class="form-group">
            <a href="{{ route('admin.produtora') }}" class="btn btn-default">Voltar</a>
