                ...
                <div class="box-body">
                    <table class="table table-bordered table-striped dataTable">
                        ...
                        @foreach($objLst as $obj)
                            <tr>
                                <td>{{ $obj->produtora_id }}</td>
                                <td style="text-align: center">{{ implode('/', array_reverse(explode('-', substr($obj->registro, 0, 10)))) . ' ' . substr($obj->registro, 11, 8) }}</td>
                                <td>{{ $obj->cpf }}</td>
                                <td>{{ $obj->cnpj }}</td>
                                <td><a href="{{ $obj->produtora_id }}/editar">{{ $obj->razao_social }}</a></td>
                                <td><a href="{{ $obj->produtora_id }}/editar">{{ $obj->responsavel }}</a></td>
                                <td>{{ $obj->email }}</td>
                                ...
                                <td style="text-align: center">
                                    @if($obj->usuario_ativo == 1)
                                        <a href="{{ $obj->produtora_id }}/editar"
                                           class="btn btn-default btn-sm"><i class="fa fa-fw fa-edit"></i></a>
                                        {{ Form::open(['method' => 'DELETE', 'route' => ['admin.produtora.deletar', $obj->produtora_id], 'style' => 'display:inline;']) }}
